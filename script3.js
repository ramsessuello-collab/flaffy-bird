...
const FOOD_BASE_POINTS = 1; // raw baseline points every food hit contributes, before multiplier
...
/* ============================
   ...
   ============================ */
let scorePopups = [];
const POPUP_LIFETIME = 50; // in delta-frame units (~0.83s of real time)
const POPUP_RISE_DISTANCE = 40; // px it floats upward over its life

const POPUP_LABEL_COLORS = {
    Junk: '#ffb84d',
    Healthy: '#8fe870',
    Lightning: '#fff066',
    Star: null // resolved dynamically using rainbowHue for a shimmering look
};
...
function spawnScorePopup(x, y, value, label, multiplierDelta) {
    scorePopups.push({
        x,
        y,
        value: Math.round(value), // displayed number, rounded for readability
        label,
        multiplierDelta, // exact delta actually applied to the multiplier, or 0 if none
        age: 0
    });
}
...
function drawScorePopups() {
    scorePopups.forEach(p => {
        ...
        // Multiplier delta line — only shown if this hit actually changed the multiplier
...
function applyMultiplierChange(effect) {
    ...
    // Round to the nearest step (not a hardcoded 0.1) so it stays exact
    // regardless of what MULTIPLIER_STEP is set to.
    multiplier = Math.round(multiplier / MULTIPLIER_STEP) * MULTIPLIER_STEP;
    multiplier = Math.round(multiplier * 100) / 100; // guard against floating-point noise
    ...
    for (let i = pipes.length - 1; i >= 0; i--) {
        ...
        if (!pipe.food.eaten) {
            ...
            if (dist < bird.size / 2 + pipe.food.radius) {
                ...
                // The multiplier in effect *right now*, before this bite can change it —
                // this is the rate used to value the points this specific hit earns.
                ...
                // maximum raw bonus points obtainable, not a guaranteed auto-hit. Any food type
                // counts, including a star hit while lightning is active.
                if (bird.invincible && bird.superType === 'lightning' && bird.lightningPipesRemaining > 0) {
                    rawPoints += (pipe.food.effect === 'shrink') ? -LIGHTNING_BONUS_POINTS : LIGHTNING_BONUS_POINTS;
...
function draw() {
    ...
    drawScorePopups(); // drawn last so popups float above everything else
}
